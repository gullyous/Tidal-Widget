...
- **Volume:** a slider under the compact controls (and in the expanded card, with a mute button) sets the playing app's volume (TIDAL, or your browser for the web player), falling back to the system volume. It appears only when a controllable audio session is found. The slider shows the effective (audible) level, so it follows keyboard volume keys and the Windows mixer; the system master acts as its ceiling, and unmuting in the widget also lifts a system mute.
